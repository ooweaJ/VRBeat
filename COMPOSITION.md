...
### Phase 3: 데이터 및 콘텐츠 검증 (진행 중)
- [x] **E-1**: 테스트 음원(`audio.mp3`) 추가
- [x] **E-2**: `song_001` 로드 및 노트 스폰 테스트
- [x] **E-3**: 슬라이스 판정 및 점수 시스템 검증

### Phase 4: 비주얼 폴리싱 & 타격감 (New!)
- [ ] **F-1: Dynamic Mesh Slicing**: 베는 평면(Saber Plane)을 계산하여 큐브 메시를 실시간 분리
- [ ] **F-2: Visual Effects**: 잘릴 때 파편 파티클(Spark) 및 세이버 검광(Trail) 추가
- [ ] **F-3: Note Material**: 색상별 Emission(발광) 머터리얼 적용 및 Bloom 효과
- [ ] **F-4: Audio/Haptic**: 베는 속도 비례 진동 강화 및 "Slash" 효과음 연결

### Phase 5: 게임 루프 완성
- [ ] **G-1**: SongSelect 씬에서 곡 목록 표시 및 선택 로직
- [ ] **G-2**: Result 씬으로 데이터 전달 및 최종 점수판 UI
